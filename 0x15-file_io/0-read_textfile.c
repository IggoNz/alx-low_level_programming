...
/**
 * read_textfile - reads the contents of a text file and prints the
 * contents to POSIX std outpu
 * @filename: the file whose contents are being read
 * @letters: the letters in the file name whose contents will be printed out
 * Return: if successful the bytes being read
 */
ssize_t read_textfile(const char *filename, size_t letters)
{
	char *buf;
